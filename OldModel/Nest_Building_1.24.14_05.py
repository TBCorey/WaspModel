#How to represent the nest in the model #Ver 05 - removing the Queen class details from Nest class
...
class Queen(object):
    ...
    def build_cell(self): #takes 0 positional arguments, but 1 was given
        if nest.free_cell_count == 0 and nest.total_cell_count < nest.max_cell_count:
    ...
    def lay_egg(self):
        while nest.free_cell_count >= round(1, 0):
            b = Brood(0, "need", "egg", "alive")
            nest.brood_list.append(b)
            nest.free_cell_count -= 1
    def forage(self):
       foraging_success = #value
        if randint(1, 100) > foraging_success:
            caterpillars_killed_today = 0.665*foraging_hours_today #need to define outside this? #RECALCULATE WITH 0.0173 instead of 0.014
            flesh_gathered = 0.0173*caterpillars_killed_today #Queens have sig. higher payload capacity!
...
class Brood(object):
    def __init__(age,food_need, dev_stage, status):
        self.age = age
        ...
        def metamorphose(self, age): #should sort brood by age first
            for b in nest.brood_list:
                ...
                if age == 40: #and also day in colony cycle, b/c they could be reproductives
                    ...
                    nest.free_cell_count += 1
    #def daily_mortality(self): - check against daily mortality rates for each dev stage; egg, larvae, pupae
...
class Worker(object):
    ...
    def forage(self):
        for w in nest.worker_list: #But how to build in foraging success rate here?
            ...
            flesh_gathered = caterpillars_killed_today*0.014#average payload capacity of workers

class Usurper(Queen): #A usurper is a queen, after all, so it should inherit the same methods, except that it also has usurp()
    def __init__(self, age, build_rate, mass): #make sure to add same args to __init__ as for Queen
        self.mass = mass
        self.age = age
        self.build_rate = choice(self.build_options)
        self.mass = round(uniform(0.086, 0.114), 3)
    def usurp(self): #generate usurper - if mass
        usurper = Usurper(0, "build", "mass")
