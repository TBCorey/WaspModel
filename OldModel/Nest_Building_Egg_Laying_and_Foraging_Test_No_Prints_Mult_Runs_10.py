    ...
    #Print statements commented out
        ...
        #Ver 11 - Reproductives finally in!
with open("Wasp_Output_Test_34.csv", mode = "w") as file:
    kills = []
    nest_sizes = []
    queen_build_rates = []
    queen_ages = []
    ...
    file.write("Kills,Nest Size,Build Rate,Queen Age, Adult Count, Total Brood, Dead Brood, Fraction Dead Brood, Fraction Dead Larvae\n")
    for i in range(len(kills)):
        file.write(str(kills[i]) + "," + str(nest_sizes[i]) + "," + str(queen_build_rates[i]) + "," + str(queen_ages[i]) + "," + str(adult_count[i]) + "," + str(total_brood_count[i]) + "," + str(dead_brood_count[i]) + "," + str(fraction_dead_brood[i]) + "," + str(fraction_dead_larvae[i]) + "\n")
