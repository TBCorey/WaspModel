    ...
    #Print statements commented out
        ...
        #Ver 08 - added total brood count, dead brood count, and fraction dead brood counts (to check mortality rates)
        #Ver 10 - more counts (fraction dead larvae), build_rate now uniform(0.28, 1.38), rather than being locked into a set of choices)
        #Ver 11 - Reproductives finally in! adult_count split into worker_count and repro_count
            ####Hey the info in Reeve et al. 1998 would be awesome to build in to determine worker dispersal
        #Ver 12 - 5.6% of larvae died before first hatching in Suzuki 1980 - try this
        #Ver 13 - That's it - OOPHAGY! 3 eggs/day killed from day of first hatching (assume day 12), for 6-10 days (randint)
            #Just make sure that the queen doesn't immediately replace these
            #OOPHAGY IS WORKING AND THERE'S A 5.6% ---DAILY--- MORTALITY RATE FOR BROOD

###Make sure that foraging starts with hatching of first egg
#What size of larvae are available when?
    #
#prey availability over the course of the season
#need different build rates for foundress and workers
#threshold to get to the next larval instar


with open("Wasp_Output_Test_40.csv", mode = "w") as file:
    kills = []
    nest_sizes = []
    queen_build_rates = []
    queen_ages = []
    ...
    for i in range(10000):
        ...
        weather_data = {"1" : [0, 0, 0, 0, 0, 5, 0, 0, 0],#May 1st
                        ...
                        "93": [5, 5, 5, 5, 5, 5, 5, 4, 5],#August 1st - emergence of reproductives (from Reeve et al. 1998)
        ...
        class Queen(object):
            ...
            def __init__(self, age, build_rate, mass, death_date, status):
                self.age = age
                self.build_rate = round(uniform(0.28, 1.38), 2)
                ...
                self.days_oophagy = randint(6, 10)
            def build_cell(self):
            ...
            def lay_egg(self):
                global day_count
                while nest.free_cell_count >= round(1, 0) and self.status == "alive" and self.eggs_laid_today < 3: #while there is at least 1 empty cell (round makes sure wasp doesn't lay in incomplete cell)
                    ...
                    self.eggs_laid_today += 1 #count so the queen only lays at most 3 eggs in a day
                    #print("Day", day_count, "-", self.eggs_laid_today, "eggs")
                self.eggs_laid_today = 0
            ...
            def oophagy(self):
                global day_count
                oophagy_count = 0
                while 12 <= day_count < (12 + self.days_oophagy) and oophagy_count < 3 and len(nest.brood_list) > 0:
                    nest.brood_list.pop()
                    #print("Oophagy!")
                    nest.free_cell_count += 1 #make sure the eggs take up space!
                    oophagy_count += 1 #count so queen only destroys 3 eggs at most in a days
            ...
            def metamorphose(self, age):
                ...
                for b in nest.brood_list:
                    if 11 <= b.age < 27: #whiteboard dump from summer had 94.4% survival rate for larvae - is this from 5.6% mortality rate?
                        ...
                        b.dev_stage = "pupae"
                    elif b.age == 40 and day_count < 93: #this will later have to break down into reproductives or workers at the end of the colony cycle
        #pssibly used sorted() here if need be: I want to sort the brood by age, so I can put the oldest brood (the ones turning 40 who will metamorphose) at the end and then pop them out of the list
                        ...
                        nest.free_cell_count += 1
                        nest.worker_count += 1
                    elif b.age == 40 and day_count >= 93:
                        r = Reproductive(40)
                        nest.repro_list.append(r)
                        nest.brood_list.remove(b)
                        nest.free_cell_count += 1
                        nest.repro_count += 1
        ...
        #print(queen.days_oophagy, "days of oophagy")
        ...
        while day_count < 123:
            ...
            if len(nest.worker_list) == 0 and queen.status == "alive":
                ...
                flesh_gathered_today = caterpillars_killed_today*0.0173*.9 #hey have you been forgetting the 10% reuse thing this whole time?
                ...
                flesh_gathered_today = caterpillars_killed_today*0.014*.9
        ...
        fraction_dead_larvae_model = (nest.larvae_dead / nest.larvae_born)
        ...
        worker_count.append(nest.worker_count)
        repro_count.append(nest.repro_count)
        ...
        fraction_dead_larvae.append(fraction_dead_larvae_model)
        
    #print(kills)
    #print(nest_sizes)

    file.write("Kills,Nest Size,Build Rate,Queen Age, Worker Count, Repro Count, Total Brood, Dead Brood, Fraction Dead Brood, Fraction Dead Larvae\n")
    for i in range(len(kills)):
        file.write(str(kills[i]) + "," + str(nest_sizes[i]) + "," + str(queen_build_rates[i]) + "," + str(queen_ages[i]) + "," + str(worker_count[i]) + "," + str(repro_count[i]) + "," + str(total_brood_count[i]) + "," + str(dead_brood_count[i]) + "," + str(fraction_dead_brood[i]) + "," + str(fraction_dead_larvae[i]) + "\n")
