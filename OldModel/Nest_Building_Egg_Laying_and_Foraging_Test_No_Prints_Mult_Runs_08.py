...
with open("Wasp_Output_Test_27.csv", mode = "w") as file:
    kills = []
    nest_sizes = []
    queen_build_rates = []
    queen_ages = []
    ...
    file.write("Kills,Nest Size,Build Rate,Queen Age, Adult Count\n")
    for i in range(len(kills)):
        file.write(str(kills[i]) + "," + str(nest_sizes[i]) + "," + str(queen_build_rates[i]) + "," + str(queen_ages[i]) + "," + str(adult_count[i]) + "\n")
