        ...
        #Ver 03 - Uses Nest_Building_Egg_Laying_and_Foraging_Test_09 script
        #Ver 04 - sensititivy analysis of sorts? Just playing around with data
        #Ver 05 - back to straight mortality, adding counter for number of workers in the population
            #should it be a day thing? like, have the avg number of workers on each day? That might be cool
            #queen builds every day? the rate i have is for nest growth over whole season, invlolves workers too
            #so let's try the Karsai values used instead! 
with open("Wasp_Output_Test_19.csv", mode = "w") as file:#started at Wasp_Output_Test_04 for mortality = 2
    kills = []
    nest_sizes = []
    queen_build_rates = []
    queen_ages = []
    ...
    for i in range(1000):
        ...
        class Nest(object):
            ...
            free_cell_count = 0
            dead_brood_count = 0
            adult_count = 0 #increment as workers eclose to get final population
            def __init__(self, species): 
                self.species = species

        class Queen(object):
            mortality_rate = 10
            build_options = [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3, 1.4]#removed hypergynous data point
            eggs_laid = 0
            def __init__(self, age, status):
                ...
                self.status = status
            def build_cell(self):
                if self.status == "alive":#and nest.free_cell_count < 1
        ...
        class Brood(object):
            def __init__(self, age):
                ...
                self.status = "alive"
         #       self.mortality_risk = randint(7, 61) #integer form of mortality rate of 7-61% (from Mead et al. 1994, avg. = 27), too high!
                self.mortality_risk = 7#start at 2 #avg from Mead et al. 1994, way too high for a daily rate, so divide by #days as larvae? go up from here
            #def feed(self, food_need, status): #if status = "larvae", b/c they're the only ones who eat
            def mortality(self):#in this version, due to mortality rate from Mead et al. 1994
                #if self.dev_stage == "larvae" and randint(1,100) <= self.mortality_risk:#if it's a larvae
                if randint(1,100) <= self.mortality_risk:
                    #print("A brood has died!")
                    nest.brood_list.remove(b)
                    nest.free_cell_count += 1
                    nest.dead_brood_count += 1
            #now instead of random death, it's because their hunger reaches a certain level!
        ...
        #            nest.brood_list.remove(b)
            ...
            def metamorphose(self):
                ...
                elif self.age == 40:#and self.food_need == round(0, 3) #this will later have to break down into reproductives or workers at the end of the colony cycle
        #possibly used sorted() here if need be: I want to sort the brood by age, so I can put the oldest brood (the ones turning 40 who will metamorphose) at the end and then pop them out of the list
            ...
            def feed(self): #let's simplify this - just give a larvae all the food it needs in one go if it's available
                ...
                if self.dev_stage == "larvae" and self.food_need > 0:
                    if flesh_gathered_today - self.food_need >= 0:#if there is enough flesh gathered to meet the food need, and there is a food need
                        flesh_gathered_today -= self.food_need
                        self.food_need = 0
                    elif self.food_need - flesh_gathered_today > 0:
       ...
       # print("Queen will live to be", queen.death_date, "days old")
        ...
        while day_count < 123:
            ...
            #print("Total flesh =", total_flesh_gathered)#currently dry weight
            #print("Feeding time!")
        ...
        #        print(b.age, b.dev_stage)
            for b in nest.brood_list:
                b.age += 1
                b.metamorphose()#this order might be a big deal?
                b.mortality()
 ...
 #       print("Total caterpillars killed:", total_caterpillars_killed)
 #       print("Final cell count:", nest.total_cell_count)
    ...
    file.write("Kills,Nest Size,Build Rate,Queen Age, Worker Count\n")
    for i in range(len(kills)):
        file.write(str(kills[i]) + "," + str(nest_sizes[i]) + "," + str(queen_build_rates[i]) + "," + str(queen_ages[i]) + "," + str(nest.adult_count) + "," + str(nest.dead_brood_count) + "," + "\n")
